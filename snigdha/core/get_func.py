...
if STRING:
    from snigdha import pro
    print("App imported from snigdha.")
else:
    pro = None
...
async def upload_media(sender, target_chat_id, file, caption, edit, topic_id):
    try:
        ...
        metadata = video_metadata(file) if file.split('.')[-1].lower() in VIDEO_EXTENSIONS else {'width': 0, 'height': 0, 'duration': 0}
...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    try:
        ...
        file = ''
        edit = await app.edit_message_text(sender, edit_id, "**Processing...**")
        if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
            ...
            if chat in saved_channel_ids:
                await app.edit_message_text(
                    ...
                    "Sorry! This channel is protected by **__SmartDev__**."
                )
                return
...
get_user_rename_preference = lambda user_id: user_rename_preferences.get(str(user_id), 'SmartDev')
...
async def send_settings_message(chat_id, user_id):
    buttons = [
        ...
        [Button.url("🚨 Report Errors", "https://t.me/TheSmartDev")]
    ]
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        try:
            ...
            await event.edit("📤 Choose your preferred upload method:\n\n__**Note:** SpyLib ⚡, built on Telethon(base), by SmartDev still in beta.__", buttons=buttons)
        except Exception as e:
    ...
    try:
        ...
        if freecheck == 1:
            reply_markup = InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💎 Get Premium to Forward", url="https://t.me/ISmartCoder")]
                ]
            )
